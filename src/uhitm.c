...
register struct monst *mtmp;
{
	...
	if(check_capacity("SUBJECT PRONOMEN_PERSONAL VERB_KOENNEN so schwer beladen nicht kämpfen.")) /* EN if(check_capacity("You cannot fight while so heavily loaded.")) */
	    goto atk_done;
	...
	if(!obj) {	/* attack with bare hands */
		...
		if (mdat == &mons[PM_SHADE] && !shade_aware(obj)) {
			...
			if (touch_petrifies(&mons[obj->corpsenm])) {
			    static const char withwhat[] = "NOUN_CORPSE"; /* EN static const char withwhat[] = "corpse"; */
			    tmp = 1;
			    hittxt = TRUE;
			    You("VERB_HIT OBJECT %s NEUES_OBJECT OBJECT KASUS_DATIV mit %s MODIFIER_CORPSE %s %s.", mon_nam(mon), /* EN You("hit %s with %s %s.", mon_nam(mon), */
...
#undef useup_eggs
			...
			if (resists_acid(mon)) {
				...
				Your("NOUN_SCHLANGENGIFT VERB_VERAETZEN OBJECT %s!", mon_nam(mon)); /* EN Your("venom burns %s!", mon_nam(mon)); */
				tmp = dmgval(obj, mon);
			}
			if (thrown) obfree(obj, (struct obj *)0);
			else useup(obj);
...
demonpet()
{
	...
	pline("Some hell-p has arrived!"); /* EN pline("Some hell-p has arrived!"); */ // TODO DE
...
struct attack *mattk;
{
	...
	while ((otmp = mdef->minvent) != 0) {
		...
		if (otmp == stealoid)	/* special message for final item */
		    pline("%s finishes taking off %s suit.", /* EN pline("%s finishes taking off %s suit.", */ // TODO DE
...
register struct attack *mattk;
{
	if (could_seduce(&youmonst, mdef, mattk))
		...
		You("VERB_MISS es."); /* EN You("miss it."); */
	 ...
	 */
		...
		case AT_TENT:
			...
			if ((dhit = (tmp > rnd(20) || u.uswallow)) != 0) {
			    ...
			    if (mattk->aatyp == AT_KICK)
				    ...
				    You("VERB_BITE OBJECT %s.", mon_nam(mon)); /* EN You("bite %s.", mon_nam(mon)); */
			    else if (mattk->aatyp == AT_STNG)
				    You("VERB_STING OBJECT %s.", mon_nam(mon)); /* EN You("sting %s.", mon_nam(mon)); */
			    else if (mattk->aatyp == AT_BUTT)
				    You("VERB_BUTT OBJECT %s.", mon_nam(mon)); /* EN You("butt %s.", mon_nam(mon)); */
			    else if (mattk->aatyp == AT_TUCH)
				    You("VERB_TOUCH OBJECT %s.", mon_nam(mon)); /* EN You("touch %s.", mon_nam(mon)); */
			    else if (mattk->aatyp == AT_TENT)
				    Your("NOUN_TENTAKELs VERB_FESTSAUGEN sich OBJECT KASUS_DATIV _an_ %s SATZKLAMMER.", mon_nam(mon)); /* EN Your("tentacles suck %s.", mon_nam(mon)); */
		...
		case AT_ENGL:
			if((dhit = (tmp > rnd(20+i)))) {
				wakeup(mon);
				if (mon->data == &mons[PM_SHADE])
				    Your("NOUN_VERSUCH, OBJECT %s zu umschließen, ist harmlos.", /* EN Your("attempt to surround %s is harmless.", */
					mon_nam(mon));
				else {
...
/*	These affect you even if they just died */
	...
	switch(ptr->mattk[i].adtyp) {
	  ...
	  case AD_MAGM:
	    ...
	    if(Antimagic) {
		shieldeff(u.ux, u.uy);
		pline("A hail of magic missiles narrowly misses you!"); /* EN pline("A hail of magic missiles narrowly misses you!"); */ // TODO DE
	    } else {
