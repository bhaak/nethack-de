...
outer_break:
	...
	if (!creation) {
		...
		if (old)
		    Sprintf(buf, " removes %s and", distant_name(old, doname)); /* EN Sprintf(buf, " removes %s and", distant_name(old, doname)); */ // TODO DE
		else
		    buf[0] = '\0';
		pline("%s%s puts on %s.", Monnam(mon), /* EN pline("%s%s puts on %s.", Monnam(mon), */ // TODO DE
