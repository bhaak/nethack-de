...
#define GDIST(x,y) (dist2(x,y,gx,gy))
	...
	if (Conflict && !resist(mtmp, RING_CLASS, 0, 0)) {
		 ...
		 */
		if (canspotmon(mtmp)) {
		    pline("%s weist OBJECT PRONOMEN_PERSONAL zurecht und sagt:", Monnam(mtmp)); /* EN pline("%s rebukes you, saying:", Monnam(mtmp)); */ // TODO DE rügen, tadeln, ermahnen
	...
	if (nix != omx || niy != omy) {
		...
		if (info[chi] & ALLOW_U) {
			if (mtmp->mleashed) { /* play it safe */
				pline("%s reißt sich von %s Leine los!", /* EN pline("%s breaks loose of %s leash!", */ // TODO DE
