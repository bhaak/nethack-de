...
struct mail_info *info;
{
    ...
    message_seen = TRUE;
    verbalize("%s, %s!  %s.", Hello(md), plname, info->display_txt); /* EN verbalize("%s, %s!  %s.", Hello(md), plname, info->display_txt); */
...
struct obj *otmp;
{
	...
	if (iflags.simplemail)
	{
		...
		while (fgets(curline, 102, mb) != NULL)
		{
		  ...
		  pline("Da steht %seine Nachricht auf der Schriftrolle.", /* EN pline("There is a%s message on this scroll.", */
		      seen_one_already ? "noch " : ""); /* EN seen_one_already ? "nother" : ""); */
		  ...
		  *msg = '\0';
		  ...
		  pline ("Diese Nachricht ist von '%s'.", curline); /* EN pline ("This message is from '%s'.", curline); */

		  msg[strlen(msg) - 1] = '\0'; /* kill newline */
		  pline ("Da steht: \"%s\".", msg); /* EN pline ("It reads: \"%s\".", msg); */
...
#ifdef SIMPLE_MAIL
bail:
	pline("Da steht aber nur Kauderwelsch."); /* bail out _professionally_ */ /* EN pline("It appears to be all gibberish."); */ /* DE Churerromanisch, hihi */
#endif
}
