...
boolean verbose;  /* give message(s) even when you can't see what happened */
{
	...
	if (tmp < rnd(20)) {
		...
		if (resists_poison(mtmp)) {
		    if (vis) pline_The("NOUN_POISON VERB_SCHEINEN OBJECT KASUS_DATIV _bei_ %s nicht zu wirken.", /* EN if (vis) pline_The("poison doesn't seem to affect %s.", */
				   mon_nam(mtmp));
		} else {
		    if (rn2(30)) {
			damage += rnd(6);
	    ...
	    if (can_blnd((struct monst*)0, mtmp,
		    ...
		    otmp)) {
		if (vis && mtmp->mcansee)
		    pline("SUBJECT %s VERB_WERDEN OBJECT KASUS_DATIV %s geblendet.", Monnam(mtmp), the(xname(otmp))); /* EN pline("%s is blinded by %s.", Monnam(mtmp), the(xname(otmp))); */
		mtmp->mcansee = 0;
