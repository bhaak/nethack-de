...
#ifdef GERMAN
		if (mtmp->mnamelth) {
			Sprintf(eos(buf), "%s", genitivattribut_zu_wort(NAME(mtmp),"NOUN_GHOST"));
		} else {
...
# endif
	...
	if (bones_ok) {
#ifdef WIZARD
	    if (!wizard || yn("Bones speichern?") == 'y') /* EN if (!wizard || yn("Save bones?") == 'y') */
#endif
		savebones(corpse);
...
*/
	...
	if (how == ESCAPED || how == ASCENDED) {
	    ...
	    if (mtmp) {
		while (mtmp) {
		    Sprintf(eos(pbuf), " und SUBJECT_IM_SATZ %s MODIFIER_VERB_PLURAL", mon_nam(mtmp)); /* EN Sprintf(eos(pbuf), " and %s", mon_nam(mtmp)); */
		    if (mtmp->mtame)
			u.urexp += mtmp->mhp;
		    mtmp = mtmp->nmon;
		}
		...
		for (i = 0; i < val->size && !done_stopprint; i++) {
			...
			otmp->onamelth = 0;
			otmp->quan = count;
			Sprintf(pbuf, "%8ld %s (im Wert von %ld %s),", /* EN Sprintf(pbuf, "%8ld %s (worth %ld %s),", */
				count, xname(otmp),
				count * (long)objects[typ].oc_cost, currency(2L));
			obfree(otmp, (struct obj *)0);
		    } else {
			Sprintf(pbuf,
				"KASUS_DATIV %8ld NOUN_GEM_COLORED_GLASS%s,", /* EN "%8ld worthless piece%s of colored glass,", */
				count, plur(count));
		    }
		    putstr(endwin, 0, pbuf);
#ifdef DUMP_LOG
	...
	if (!done_stopprint) {
	    ...
	    if (dump_fp) {
	      dump("", pbuf);
	      Sprintf(pbuf, "Todesursache: %s", killer); /* EN Sprintf(pbuf, "Killer: %s", killer); */
	      dump("", pbuf);
	    }
#endif
	}
