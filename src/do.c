...
boolean pushing;
{
	if (!otmp || otmp->otyp != BOULDER)
		...
		if (fills_up && u.uinwater && distu(rx,ry) == 0) {
		    ...
		    Dich("SUBJECT_IM_SATZ VERB_TREFFEN flüssige NOUN_LAVA%c", /* EN You("are hit by molten lava%c", */
			Fire_resistance ? '.' : '!');
			burn_away_slime();
...
#endif
		You_hear("ein allgegenwärtiges Heulen und Zähneklappern."); /* EN You_hear("groans and moans everywhere."); */
...
long timeout;
{
	...
	if (is_rider(&mons[body->corpsenm]))
	    Du_fuehlst_dich("weniger bedrängt."); /* EN You_feel("less hassled."); */
