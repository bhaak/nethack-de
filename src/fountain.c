...
const char *what;
{
    You("VERB_SCHWEBEN hoch ¸ber OBJECT ARTIKEL_BESTIMMTER %s.", what); /* EN You("are floating high above the %s.", what); */
}
...
dowaterdemon() /* Water demon */
{
	...
	if((mtmp = makemon(&mons[PM_WATER_DEMON],u.ux,u.uy, NO_MM_FLAGS))) {
		...
		Du_spuerst("die Pr‰senz des Bˆsen."); /* EN You_feel("the presence of evil."); */
...
dowaternymph() /* Water Nymph */
{
	...
	if(!(mvitals[PM_WATER_NYMPH].mvflags & G_GONE) &&
	   (mtmp = makemon(&mons[PM_WATER_NYMPH],u.ux,u.uy, NO_MM_FLAGS))) {
		...
		if (!Blind)
		   pline("Eine groﬂe Luftblase steigt zur Oberfl‰che auf und zerplatzt."); /* EN pline("A large bubble rises to the surface and pops."); */
		else
...
drinkfountain()
{
	...
	if (fate < 10) {
		...
		case 29: /* Scare */ {
			...
			pline("Von dem Wasser VERB_BEKOMMEN SUBJECT_IM_SATZ PRONOMEN_PERSONAL ¸blen Mundgeruch!"); /* EN pline("This water gives you bad breath!"); */
			for(mtmp = fmon; mtmp; mtmp = mtmp->nmon)
			    if(!DEADMONSTER(mtmp))
