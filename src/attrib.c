...
const struct innate {
	...
	const char *gainstr, *losestr;
		     ...
		     {   1, &(HFast), "", "" },
		     {  10, &(HSearching), "aufmerksam", "unaufmerksam" }, /* EN {  10, &(HSearching), "perceptive", "" }, */
		     {	 0, 0, 0, 0 } },
	...
	elf_abil[] = { {	4, &(HSleep_resistance), "wach", "müde" }, /* EN elf_abil[] = { {	4, &(HSleep_resistance), "awake", "tired" }, */
		     {	 0, 0, 0, 0 } },
...
{			    /* negative => conditional (msg if change made) */
	...
	if ((ndx == A_INT || ndx == A_WIS)
		...
		if (msgflg == 0)
		    Your("%s-NOUN_BEDECKUNG drückt kurz etwas, aber das lässt gleich wieder nach.", fugenwort(body_part(HEAD))); /* EN Your("cap constricts briefly, then relaxes again."); */
		return FALSE;
	}
	...
	if (msgflg <= 0)
	    Du_fuehlst_dich("%s%s!", /* EN You_feel("%s%s!", */
	...
	while (abil || rabil) {
		...
		if(oldlevel < abil->ulevel && newlevel >= abil->ulevel) {
			...
			if(!(*(abil->ability) & INTRINSIC & ~mask)) {
			    if(*(abil->gainstr))
				Du_fuehlst_dich("%s!", abil->gainstr); /* EN You_feel("%s!", abil->gainstr); */
			}
