...
const char *drainer;	/* cause of death, if drain should be fatal */
{
	...
	if (u.ulevel > 1) {
		pline("%s Stufe %d.", Goodbye(), u.ulevel--); /* EN pline("%s level %d.", Goodbye(), u.ulevel--); */
...
{		/*	(false for potion of gain level)      */
	...
	if (!incr) Du_fuehlst_dich("erfahrener."); /* EN if (!incr) You_feel("more experienced."); */
	num = newhp();
	u.uhpmax += num;
	u.uhp += num;
	if (Upolyd) {
	...
	if (u.ulevel < MAXULEV) {
	    ...
	    pline("Willkommen auf Erfahrungsstufe %d.", u.ulevel); /* EN pline("Welcome to experience level %d.", u.ulevel); */
