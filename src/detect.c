...
register struct obj	*sobj;
{
	...
	known = TRUE;
	You("%s %s ganz in der Nähe.", sobj ? "VERB_RIECHEN" : "VERB_SPUEREN", what); /* EN You("%s %s nearby.", sobj ? "smell" : "sense", what); */
	if (sobj && sobj->blessed) {
...
int		class;		/* an object class, 0 for all */
{
	...
	You("VERB_SPUEREN OBJECT %s in der Nähe.", stuff); /* EN You("sense %s nearby.", stuff); */
	return 0;
    }
...
struct obj *obj;
{
	...
	if (!obj->spe) {
	    ...
	    switch(rnd(6)) {
	    case 1 : You("VERB_VERINNERLICHEN die strukturelle Essenz gleißender Lavablasen."); /* EN case 1 : You("grok some groovy globs of incandescent lava."); */
		break;
