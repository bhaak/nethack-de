...
dodrink()
{
	...
	if (IS_SINK(levl[u.ux][u.uy].typ)) {
		if (yn("Von der Spüle trinken?") == 'y') { /* EN if (yn("Drink from the sink?") == 'y') { */
			drinksink();
			return 1;
		}
	}
	...
	register struct obj	*otmp;
{
	...
	case POT_GAIN_ENERGY:			/* M. Stephenson */
		{	register int num;
			if(otmp->cursed)
			    Du_fuehlst_dich("dröge."); /* EN You_feel("lackluster."); */
			else
			    pline("Magische Energie strömt KASUS_AKKUSATIV durch PRONOMEN_POSSESSIV NOUN_KOERPER."); /* EN pline("Magical energies course through your body."); */
	...
	if (potion->otyp == POT_OIL) {
		...
		if (catch_lit(obj)) {
		    ...
		    pline("Das brennende Öl %s KASUS_AKKUSATIV %s.", /* EN pline_The("burning oil %s %s.", */
			    obj->oeroded == MAX_ERODE ? "zerstört" : "beschädigt", /* EN obj->oeroded == MAX_ERODE ? "destroys" : "damages", */
			    yname(obj));
