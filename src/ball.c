...
ballfall()
{
	...
	if(gets_hit){
		...
		if (uarmh) {
			...
			pline("Glücklicherweise VERB_TRAGEN SUBJECT_IM_SATZ PRONOMEN_PERSONAL einen schützenden Helm."); /* EN pline("Fortunately, you are wearing a hard helmet."); */
			dmg = 3;
	...
	if (u.utrap && u.utraptype != TT_INFLOOR) {
...
#endif
		{
		    Your("%s %s wurde schwer verletzt.", /* EN Your("%s %s is severely damaged.", */
					...
					body_part(LEG));
		    losehp(2, "erlag einer Bärenfallenbefreiungsaktionsbeinverletzung", /* EN losehp(2, "leg damage from being pulled out of a bear trap", */
...
drag_down()
{
	...
	if (forward) {
		...
		if(rn2(2)) {
			...
			losehp(rnd(20), "durch einen Zusammenstoß mit einer Eisenkugel", KILLED_WITHOUT_PREPOSITION); /* EN losehp(rnd(20), "iron ball collision", KILLED_BY_AN); */
